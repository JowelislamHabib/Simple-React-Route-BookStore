...
const Wishlist = ({ sortingType }) => {
  ...
  useEffect(() => {
    if (sortingType) {
      if (sortingType === "pages") {
        const sortedData = [...storeWishlist].sort(
          (a, b) => a.totalPages - b.totalPages,
        );
        setFilteredWishlist(sortedData);
      } else if (sortingType === "ratings") {
        const sortedData = [...storeWishlist].sort((a, b) => a.rating - b.rating);
        setFilteredWishlist(sortedData);
      }
    }
  ...
  if (storeWishlist.length === 0) {
    return (
      <div className="h-[50vh] bg-base-200 rounded-2xl flex items-center justify-center ">
        <h2 className="font-bold text-3xl opacity-50">No wishlist book found</h2>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-2xl font-bold mb-8">Books in Wishlist ({filteredwishlist.length})</h2>
